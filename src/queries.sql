...
-- Retrieves a list of health checks by execution group id.
select checks.*, coalesce(states.failing_count, 0), coalesce(states.response_count, 0), coalesce(states.state_name, 'INITIALIZING') from checks left outer join check_states as states on (checks.id = states.check_id) where checks.execution_group_id=:execution_group_id::UUID and checks.customer_id=:customer_id::UUID;
